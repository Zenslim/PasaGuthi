...
function RenderContent({ data }) {
  ...
  return (
    <div className="prose prose-invert max-w-none">
      {blocks.map((b, i) => {
        if (b.type === "header") {
          return <h2 key={i} dangerouslySetInnerHTML={{ __html: b.data.text }} />;
        }
        if (b.type === "paragraph") {
...
export default function PostPage({ post }) {
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      <Head>
        <title>{post.title} – Pasaguthi</title>
      </Head>
      ...
      {post.excerpt ? <p className="text-zinc-300 mb-8">{post.excerpt}</p> : null}
